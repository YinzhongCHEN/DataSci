...
def execute_sql_with_all_support(sql: str, table_to_path: dict, order_by=None,
    ...
    # 如果是窗口函数
    if is_window:
        print(f"[计时] 1. 解析 Window SQL 耗时：{(t1 - t0)*1000.0:.3f} ms")
    ...
    # 原来的写法会报错：list + set；改为先转换成 list
        ...
        t3 = time.time()
        print(f"[计时] 2. 提取窗口要加载的列 耗时：{(t3 - t2)*1000.0:.3f} ms → usecols={usecols}")

        # 4. 只加载这些列到张量
        t4 = time.time()
        tensors, df_subset = load_data(csv_path, usecols=usecols, nrows=nrows)
        t5 = time.time()
        print(f"[计时] 3. 加载列到张量 耗时：{(t5 - t4)*1000.0:.3f} ms → 行数={df_subset.shape[0]}")
        ...
        t7 = time.time()
        print(f"[计时] 4. 张量 WHERE+投影 耗时：{(t7 - t6)*1000.0:.3f} ms  → 过滤后行数={filtered[grp_col].shape[0]}")
        ...
        t9 = time.time()
        print(f"[计时] 5. 张量 上窗口函数 ROW_NUMBER 耗时：{(t9 - t8)*1000.0:.3f} ms")
    ...
    if is_agg:
        print(f"[计时] 1. 解析 Aggregate SQL 耗时：{(t11 - t10)*1000.0:.3f} ms")
        ...
        t13 = time.time()
        print(f"[计时] 2. 提取聚合要加载的列 耗时：{(t13 - t12)*1000.0:.3f} ms → usecols={usecols}")
        ...
        t15 = time.time()
        print(f"[计时] 3. 加载列到张量 耗时：{(t15 - t14)*1000.0:.3f} ms → 行数={df_subset_p.shape[0]}")
        ...
        if where_pred_p is not None:
            ...
            t17 = time.time()
            print(f"[计时] 4. 张量 WHERE 过滤 耗时：{(t17 - t16)*1000.0:.3f} ms  → 过滤后行数={col_tensor.shape[0]}")
        else:
            # 直接拿整列
        ...
        t19 = time.time()
        print(f"[计时] 5. 张量聚合 ({agg_func}) 耗时：{(t19 - t18)*1000.0:.3f} ms")
    ...
    print(f"[计时] 1. 非 Window/非 Agg SQL，退回普通解析")
    # a) 先解析 SELECT/WHERE
    if table_name_p is None:
        # 意味着 parse_sql_with_agg 也不认识，抛错
        raise ValueError(f"无法解析的 SQL: {sql}")
    print(f"[计时] 解析 SELECT/WHERE 耗时：{(t11 - t10)*1000.0:.3f} ms")
    ...
    t21 = time.time()
    print(f"[计时] 2. 提取常规投影要加载的列 耗时：{(t21 - t20)*1000.0:.3f} ms → usecols={usecols}")

    # c) 加载张量
    t22 = time.time()
    tensors_r, df_subset_r = load_data(csv_path, usecols=usecols, nrows=nrows)
    t23 = time.time()
    print(f"[计时] 3. 加载列到张量 耗时：{(t23 - t22)*1000.0:.3f} ms → 行数={df_subset_r.shape[0]}")
    ...
    if use_cuda:
        ...
        print(f"[计时] 4. 张量 WHERE+投影（GPU） 耗时：{sel_ms:.3f} ms")
    else:
        t24 = time.time()
        result_tensors = tensor_select(tensors_r, select_cols_p, where_pred_p)
        sel_ms = (time.time() - t24)*1000.0
        print(f"[计时] 4. 张量 WHERE+投影（CPU） 耗时：{sel_ms:.3f} ms")
    ...
    if not order_by:
        ...
        t26 = time.time()
        print(f"[计时] 5. 封装成 DataFrame 耗时：{(t26 - t25)*1000.0:.3f} ms, 返回行数={Mf}")
    ...
    t28 = time.time()
    print(f"[计时] 5. 构建排序用 DataFrame 耗时：{(t28 - t27)*1000.0:.3f} ms")
